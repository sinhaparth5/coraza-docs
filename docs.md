...
- **Threat-intel auto-sync & webhooks** — pull external IP blocklists and send generic JSON, Slack,
  or Discord alerts. See
...
- **Prometheus metrics & request logging** — per-cause counters, async database logging, an optional
...
make tag VERSION=v1.0.0   # annotated git tag + push (triggers the GitHub Actions release workflow)
```
...
The dashboard lives at `/admin` and is behind session-cookie login. Protection, routing, and
integration changes apply live; switching the database backend is the exception and requires a
restart. Each page is HTMX-driven, so adding/removing a row swaps just that part of the page. This
section also lists the underlying HTTP routes.

:::info[How "implement" works here]
Each dashboard form maps to a route that writes the change to the active database and, where needed,
**hot-reloads** the relevant subsystem (registry, blocklist, geo, WAF engine, or rate limiter).
:::
...
*only* thing that needs SIGHUP; services, bot settings, and rate limits already hot-reload on save.
Changing `--db-driver`/`--db` is a separate startup operation and requires a normal restart.
...
A per-IP token bucket lives in memory; its state is snapshotted to the active database **every 10
seconds** and restored on startup, so limits survive restarts. Idle buckets (no traffic for 5
minutes) are reclaimed by a background janitor so the bucket map stays bounded despite an unbounded
number of distinct client IPs over time. This is the right choice for a single node.
...
Global rate limiting runs **early** in the pipeline (after IP/geo blocks and bot challenge, before
the WAF), so throttled clients are rejected cheaply. See
[Architecture](/docs/overview/architecture) for the full pipeline order.
...
Request logs accumulate in the active database. Pruning is **not** automatic inside the running
server; it is a separate one-shot command so maintenance does not contend with live request traffic.

Run it from cron or the bundled systemd timer:

```bash
coraza-waf-mod prune --db-driver sqlite \
  --db /var/lib/coraza-waf-mod/waf.db --retention 30 --vacuum
```

The installer sets up `coraza-waf-mod-prune.service` + `.timer` to run every 15 days automatically.
Deletes happen in batches (2000 rows) with short pauses. With `--vacuum`, SQLite performs a full
rebuild and WAL truncate, Postgres runs `VACUUM`, and MySQL does nothing because it has no
database-wide equivalent. Do not run SQLite vacuuming inside the live server process.
...
of the database-backed request log the dashboard reads from — for tooling that expects one: `fail2ban`,
...
elsewhere (e.g. `coraza-waf-mod prune` + a systemd timer for structured request history) — no
...
| `coraza_log_queue_depth` | Gauge | — | Current buffered request-log entries waiting to be written to the active database (see [State & storage](/docs/overview/architecture#state--storage)). Read live on every scrape, not polled. |
...
# HELP coraza_log_queue_depth Current number of request-log entries buffered waiting to be written to the database.
...
## Repeated-request verdict cache

Each WAF engine keeps a 4096-entry LRU cache of recent verdicts for byte-identical anonymous
requests. The key covers method, host, normalized path, query string, and a SHA-256 body fingerprint;
entries expire after five seconds. This absorbs repeated scanner or flood traffic without applying
heuristics to merely similar requests. Any request with a `Cookie` or `Authorization` header bypasses
the cache so authenticated identities can never share a verdict.
...
- Each delivery is an `HTTP POST` with `Content-Type: application/json` and a 5-second client
  timeout. Payload shape depends on the destination:

  | Destination | Payload |
  |---|---|
  | `generic` | The full JSON-marshaled request-log row, suitable for a custom receiver or SIEM. |
  | `slack` | A colored Block Kit attachment for a Slack incoming webhook. |
  | `discord` | A colored rich embed for a Discord incoming webhook. |
...
- **Authentication is a shared secret, not a computed signature.** For a generic receiver, if
  `webhook_secret` is set, delivery carries it verbatim as an `X-WAF-Secret` header — compare that
  header against the receiver's own copy rather than expecting an HMAC. Slack and Discord incoming
  webhooks authenticate through their URL and do not use this setting.

Configure it under **Settings → Webhooks** (`POST /admin/settings/webhook` with
`webhook_destination_type`, `webhook_url`, `webhook_secret`, `webhook_enabled=1`, and one or more
`webhook_events` checkboxes). Save first, then use **Send test alert** to deliver a synthetic blocked
request through the saved destination; the test deliberately ignores the enabled toggle and event
filters.

### Receiving a generic delivery
